...
export default {
  ...
  async fetch(request, env) {
    ...
    if (pathname.startsWith("/admin/")) {
      ...
      if (pathname === "/admin/emails/read-all" && method === "PATCH") return handlers.handleAdminEmailsMarkAllRead(url, env.DB);
      if (pathname.startsWith("/admin/emails/") && method === "GET") return handlers.handleAdminEmailDetail(pathname, env.DB);
      if (pathname.startsWith("/admin/emails/") && pathname.endsWith("/read") && method === "PATCH") return handlers.handleAdminEmailMarkRead(pathname, env.DB);
